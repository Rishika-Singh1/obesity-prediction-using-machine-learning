...
# Prepare data (drop target)
X_hc = df.drop('NObeyesdad', axis=1)
X_hc = pd.get_dummies(X_hc, drop_first=True)
...
# linkage method: 'ward' minimizes variance (best for numeric + encoded data)
Z = linkage(X_hc, method='ward')

dendrogram(Z)
plt.show()

# FIT HIERARCHICAL CLUSTERING MODEL (Agglomerative)
optimal_clusters = 4   # you may choose based on dendrogram break point
print(f"Selected number of clusters for HC: {optimal_clusters}")
...
df['HC_Cluster'] = hc_model.fit_predict(X_hc)

print("\nHierarchical cluster labels added as 'HC_Cluster'.")

# CLUSTER vs OBESITY TABLE
hc_obesity_table = pd.crosstab(df['HC_Cluster'], df['NObeyesdad'])
print("\nHierarchical Cluster vs Obesity Level Table:")
print(hc_obesity_table)

# VISUALIZATION (Weight vs Age)
plt.figure(figsize=(10,6))
sns.scatterplot(data=df, x='Weight', y='Age', hue='HC_Cluster', palette='tab10', alpha=0.8)
plt.title(f"Hierarchical Clusters on Weight vs Age (Clusters={optimal_clusters})")
plt.xlabel("Weight (kg)")
plt.ylabel("Age")
...
print("Number of clusters selected: 4")
print("K-Means clusters show strong alignment with obesity categories.")

print("\nHierarchical Clustering:")
print("Number of clusters selected: 4")
print("Hierarchical clusters also show clear separation of obesity levels.")

print("\nConclusion:")
print("Both clustering techniques identified similar lifestyle-based patterns.")
print("This confirms the presence of meaningful natural groupings in the dataset.")
